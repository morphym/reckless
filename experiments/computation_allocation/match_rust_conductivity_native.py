#!/usr/bin/env python3
"""Play color-swapped games between heuristic Physarum and native Reckless."""
import argparse
import json
...
def main():
    ...
    artifact = {'protocol': {'flow_engine': str(args.flow_engine.resolve()),
                             ...
                             'qnodes': args.qnodes}, 'games': games}
    ...
    with args.output.with_suffix('.pgn').open('w') as handle:
        for record in games:
            ...
            game.headers['White'] = 'Heuristic Physarum' if record['flow_color'] == 'white' else 'Native Reckless'
            game.headers['Black'] = 'Native Reckless' if record['flow_color'] == 'white' else 'Heuristic Physarum'
            game.headers['FEN'] = fen
            game.headers['SetUp'] = '1'
            # Rebuild from the initial FEN, since setup(final) would reverse the game.
            game = chess.pgn.Game()
            game.setup(chess.Board(fen))
            game.headers.update({'Result': record['result'], 'White': 'Heuristic Physarum' if record['flow_color'] == 'white' else 'Native Reckless',
                                 'Black': 'Native Reckless' if record['flow_color'] == 'white' else 'Heuristic Physarum'})
            node = game
